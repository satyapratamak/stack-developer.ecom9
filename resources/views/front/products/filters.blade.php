...
<div class="col-lg-3 col-md-3 col-sm-12">
    ...
    <?php
    $filterAvailable = ProductFilters::filterAvailable($filter['id'], $categoryDetails['categoryDetails']['id'])
    ?>
    @if ($filterAvailable && count($filter['filter_values']) >0)
    <div class="facet-filter-associates">
        <h3 class="title-name">{{ $filter['filter_name'] }}</h3>
        <form class="facet-form" action="#" method="post">
            <div class="associate-wrapper">
                @foreach ($filter['filter_values'] as $value)
                <input type="checkbox" class="check-box {{$filter['filter_column'] }}" value="{{ $value['filter_value'] }}" name="{{$filter['filter_column'] }}[]" id="{{ $value['filter_value'] }}">
